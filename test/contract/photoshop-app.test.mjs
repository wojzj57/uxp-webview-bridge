...
test("host dispatches app, Documents, TextFonts, action tree, and Preferences through references", async () => {
  ...
  let actionSet;
  const action = {
    typename: "Action", id: 2, index: 1, name: "Step",
    delete() { actionCalls.push("action.delete"); },
    duplicate() { actionCalls.push("action.duplicate"); return { ...this, id: 3, name: "Step copy", parent: actionSet }; },
    play() { actionCalls.push("action.play"); }
  };
  actionSet = {
    typename: "ActionSet", id: 1, index: 1, name: "Set", actions: [action],
    delete() { actionCalls.push("actionSet.delete"); },
    duplicate() {
      actionCalls.push("actionSet.duplicate");
      const copy = { ...this, id: 4, name: "Set copy", actions: [] };
      copy.actions = [{ ...action, id: 5, parent: copy }];
      return copy;
    },
    play() { actionCalls.push("actionSet.play"); }
  };
  ...
  try {
    ...
    await dispatchPhotoshopCall("app.bringToFront", [appRef]);
    await dispatchPhotoshopCall("app.updateUI", [appRef]);
    ...
    await dispatchPhotoshopCall("actionSet.propertySet", [setRef, "name", "Renamed Set"]);
    assert.equal(actionSet.name, "Renamed Set");
    assert.equal((await dispatchPhotoshopCall("actionSet.duplicate", [setRef])).type, "ActionSet");
    await dispatchPhotoshopCall("actionSet.play", [setRef]);
    await dispatchPhotoshopCall("actionSet.delete", [setRef]);
    const actionRef = ref("Action", actions.memberIds[0]);
    await dispatchPhotoshopCall("actionObject.propertySet", [actionRef, "name", "Renamed Action"]);
    assert.equal(action.name, "Renamed Action");
    assert.equal((await dispatchPhotoshopCall("actionObject.duplicate", [actionRef])).type, "Action");
    await dispatchPhotoshopCall("actionObject.play", [actionRef]);
    await dispatchPhotoshopCall("actionObject.delete", [actionRef]);
    assert.deepEqual(actionCalls, [
      "actionSet.duplicate", "actionSet.play", "actionSet.delete",
      "action.duplicate", "action.play", "action.delete"
    ]);
    ...
    assert.deepEqual(modal, [
      "app.createDocument", "app.bringToFront", "app.updateUI", "app.open",
      "actionSet.propertySet", "actionSet.duplicate", "actionSet.play", "actionSet.delete",
      "actionObject.propertySet", "actionObject.duplicate", "actionObject.play", "actionObject.delete",
